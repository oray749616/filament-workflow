...
class DeepSeekController extends Controller
{
    ...
    public function chat(Request $request)
    {
        ...
        $promptTemplate = <<<EOT
            ...
            ## 输出格式
            请输出纯文本格式，保证所有数字和符号完整显示，尤其注意不要将年份（如2000年）分隔开。每篇文章之间用"---"分隔。
            EOT;
    ...
    public function streamChat(Request $request)
    {
        ...
        $promptTemplate = <<<EOT
            ...
            ## 二、任务概述
            基于上述输入材料，创作 {num} 篇高质量文章。

            ## 三、写作要求
            1. **渠道优化**：针对「{channels}」渠道进行内容和格式优化。  
            2. **主题方向**：围绕「{direction}」展开，确保主题鲜明。  
            3. **内容规范**：严格遵循「{requirements}」中的内容要求。  
            4. **字数控制**：每篇文章字数为原文的 90%–110%，并保持合理范围。  
            5. **标题吸引**：为每篇文章拟写一个富有吸引力的标题。  
            6. **结构清晰**：段落分明，逻辑清晰，便于阅读。 

            ## 四、SEO优化要求
            - 自然融入以下关键词：{seo_keywords}
            - 在文章末尾输出：  
            - 推荐的 SEO 关键词列表  
            - 每个关键词的密度建议
            ...
            ## 六、输出格式
            - 以纯文本形式输出，确保数字和符号（如“2000年”）完整无拆分。  
            - 各篇文章之间以 `---` 分隔。  
            EOT;
